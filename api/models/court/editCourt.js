...
const editCourt = async (req, res) => {
  ...
  try {
    ...
    const courtQuery = `
      UPDATE courts 
      SET court_name = $1, court_type = $2, venue_overview = $3, rules_of_venue = $4 
    ...
    if (location) {
      const locationQuery = `
        UPDATE locations 
        SET country = $1, city = $2, location_link = $3 
        WHERE court_id = $4;
      `;
    ...
    if (venuePrice) {
      const venuePriceQuery = `
        UPDATE venue_price 
        SET starting_price = $1, max_guests = $2, additional_guests = $3, price_of_additional_guests = $4 
        WHERE court_id = $5
      `;
